...
export default function DashboardPage() {
  ...
  const [formData, setFormData] = useState({ 
    name: '', 
    email: '', 
    password_key: '', // Novo campo de senha
    phone: '', 
    plan_total: '', 
    services: [] as string[] 
  });
  ...
  const fetchAdminData = async () => {
    try {
      const { data: invData } = await supabase
        .from('invoices')
        .select('*, subscribers(name, avatar_color, phone)')
        .order('due_date', { ascending: false });

      if (invData) {
        const sortedInvoices = invData.sort((a, b) => {
  ...
  const handleGenerateInvoices = async () => {
    if (!confirm("Gerar faturas do mês para todos?")) return;
    setProcessing(true);
    try {
  ...
  const handleCopyGeneralReport = () => {
    ...
    if (pendingInvoices.length === 0) return alert("Ninguém deve nada! Tudo pago.");

    const mes = pendingInvoices[0].month_ref;
    
    let text = `📢 *RESUMO DE FATURAS - ${mes.toUpperCase()}* 📢\n\n`;
    pendingInvoices.forEach(inv => {
      text += `👤 *${inv.subscribers.name}*: R$ ${Number(inv.amount).toFixed(2)}\n`;
    });
    
    text += `\n📲 *Acesso ao Sistema (Senhas):*\n${APP_URL}\n\n`;
    text += `💳 *DADOS PARA PIX:*\n`;
    text += `🔑 Chave: ${PIX_KEY}\n`;
    text += `🏦 Banco: ${PIX_BANK}\n`;
    text += `👤 Nome: ${PIX_NAME}`;
    ...
    alert("Relatório com dados bancários copiado!");
  };

  const handleWhatsapp = (inv: any) => {
    const phone = inv.subscribers?.phone?.replace(/\D/g, '');
    if (!phone) return alert("Telefone não cadastrado.");
    const text = `Fala ${inv.subscribers.name}! 👋%0A%0AFatura *${inv.month_ref}* disponível.%0AValor: *R$ ${Number(inv.amount).toFixed(2)}*%0A%0A🔑 PIX: ${PIX_KEY}%0A🏦 ${PIX_BANK}%0A%0AApp: ${APP_URL}`;
    window.open(`https://wa.me/${phone}?text=${text}`, '_blank');
  };

  // --- CRUD USUÁRIOS (AGORA COM LOGIN E SENHA) ---
  const openUserModal = (userToEdit: any = null) => {
    if (userToEdit) {
      ...
      setFormData({
        ...
        password_key: userToEdit.password_key || '1234', // Carrega a senha atual
        phone: userToEdit.phone || '', 
        plan_total: userToEdit.plan_total, 
  ...
  const toggleServiceInForm = (serviceName: string) => {
    setFormData(prev => {
      ...
      if (exists) return { ...prev, services: prev.services.filter(s => s !== serviceName) };
      else return { ...prev, services: [...prev.services, serviceName] };
    });
  };
  ...
  const saveUser = async (e: React.FormEvent) => {
    ...
    // Se o email estiver vazio, gera um automático, senão usa o digitado
    const finalEmail = formData.email || formData.name.toLowerCase().replace(/\s/g,'')+'@stream.com';
    // Se a senha estiver vazia, usa '1234', senão usa a digitada
    ...
    const payload = {
      ...
      plan_total: parseFloat(formData.plan_total), 
      services: formData.services,
      password_key: finalPass, // Salva a senha definida pelo admin
      avatar_color: editingId ? undefined : 'bg-blue-500'
    };
            ...
            <div className="flex justify-between items-center">
              ...
              {isAdmin && (
                <div className="flex gap-2">
                  <button onClick={handleCopyGeneralReport} className="bg-green-600 hover:bg-green-500 text-white text-xs md:text-sm px-3 py-2 rounded-full shadow-lg flex items-center gap-2"><ClipboardCopy size={16}/> Cobrar Todos (Grupo)</button>
            ...
            <div className="glass-card p-6 rounded-2xl bg-gradient-to-br from-emerald-900/20 to-green-900/10 border-emerald-500/20">
               <div className="flex flex-col gap-4">
                  ...
                  <div className="flex flex-col gap-1">
                    <p className="text-[10px] text-gray-400 uppercase tracking-wide">Chave PIX (Celular)</p>
                    <div className="flex items-center gap-2 bg-black/30 p-2.5 rounded-xl border border-white/5 mb-2">
                      <code className="text-emerald-400 font-mono text-sm flex-1">{PIX_KEY}</code>
                      <button onClick={() => copy(PIX_KEY)} className="p-1.5 hover:bg-white/10 rounded-lg text-gray-300"><Copy size={16} /></button>
                    </div>
                    <div className="flex flex-col text-xs text-gray-300 px-1 gap-0.5">
                      <p>🏦 {PIX_BANK}</p>
                      <p>👤 {PIX_NAME}</p>
                    </div>
                  </div>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="glass-card w-full max-w-lg p-6 rounded-3xl animate-float border border-white/20 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={saveUser} className="space-y-3">
              ...
              {/* NOVOS CAMPOS DE LOGIN E SENHA */}
              <div className="grid grid-cols-2 gap-3">
                 <div>
                    <label className="text-xs text-gray-400 ml-1 mb-1 block">Login / Email</label>
                    <div className="relative">
                      <Mail className="absolute left-3 top-3 text-gray-500" size={16} />
                      <input placeholder="email@login.com" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} className="glass-input w-full pl-9 p-2.5 rounded-xl text-sm" />
                    </div>
                 </div>
                 <div>
                    <label className="text-xs text-gray-400 ml-1 mb-1 block">Senha Acesso</label>
                    <div className="relative">
                      <Lock className="absolute left-3 top-3 text-gray-500" size={16} />
                      <input placeholder="1234" value={formData.password_key} onChange={e => setFormData({...formData, password_key: e.target.value})} className="glass-input w-full pl-9 p-2.5 rounded-xl text-sm" />
                    </div>
                 </div>
              </div>

              <input required type="number" step="0.01" placeholder="Valor Mensal (R$)" value={formData.plan_total} onChange={e => setFormData({...formData, plan_total: e.target.value})} className="glass-input w-full p-3 rounded-xl" />
              <input required placeholder="Celular (WhatsApp)" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} className="glass-input w-full p-3 rounded-xl" />
